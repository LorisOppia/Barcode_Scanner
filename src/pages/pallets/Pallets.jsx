...
const Pallets = (props) => {
  ...
  const [showToastCancelButton, setShowToastCancelButton] =
    React.useState(false); //barra sotto per annulla
  ...
  const startScan = async (pulsante) => {
    ...
    if (result.hasContent) {
      ...
      if (pulsante === 1) {
        setNewCode(result.content);
        setShowAlertNewQuantity(true);
      }
    }
  };
    ...
    return (
      <IonPage>
        ...
        <IonContent>
          ...
          <IonItem>
            <IonLabel text-wrap>QR: {code}</IonLabel>
            <IonButton
              onClick={() => checkPermission(0)}
              size="medium"
              expand="block"
              slot="end"
            >
          ...
          <IonItem>
            <IonLabel text-wrap>Nuova quantità: {quantity}</IonLabel>
          </IonItem>

          <IonItem>
            <IonLabel text-wrap>QR: {newCode}</IonLabel>
            <IonButton
              onClick={() => {
                checkPermission(1);
              }}
          ...
          <IonItem>
            <IonLabel text-wrap>Quantità sottratta: {newQuantity}</IonLabel>
          </IonItem>
        </IonContent>
